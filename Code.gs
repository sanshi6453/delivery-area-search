...
function doGet() {
  ...
  const required = ['ID','都道府県','市町村','市町村よみ','町名','町名よみ','郵便番号','配達店舗','判定','配達曜日','検索方法','注意事項','有効','元データ表記'];
  ...
  const rows = values.slice(1).filter(r => r.some(v => String(v).trim())).map(r => ({
    ...
    town:r[col['町名']], townReading:r[col['町名よみ']], postalCodes:String(r[col['郵便番号']] || '').split(/[,、／/\s]+/).map(v => v.replace(/\D/g, '')).filter(v => v.length === 7), store:r[col['配達店舗']], status:r[col['判定']], deliveryDay:r[col['配達曜日']],
